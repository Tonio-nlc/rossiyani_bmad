-- ============================================================
-- B3 cas 3 — хото́ть → хоте́ть (faute de lettre)
-- Surface rattachée : хо́чешь — cible linguistique confirmée.
-- En base live : хоте́ть ABSENT ; « хотеть » (nu) présent
...
begin
  select id into v_bad_id from lemmas where form = 'хото́ть';
  select id, form into v_good_id, v_good_form from lemmas where form = 'хоте́ть';
  if v_good_id is null then
    select id, form into v_good_id, v_good_form from lemmas where form = 'хотеть';
  end if;

  if v_bad_id is null then
    raise notice 'RAPPORT : lemme fautif хото́ть absent — rien à faire.';
    return;
  end if;
  ...
  raise notice 'RAPPORT-AVANT хото́ть id=% → cible % id=% | wf=% lcl=% uv=% ec=% lk=%',
    v_bad_id, coalesce(v_good_form, 'хоте́ть (rename)'), v_good_id, v_wf, v_lcl, v_uv, v_ec, v_lk;
end $$;
...
begin
  ...
  if v_bad_id is null then
    raise notice 'SKIP : хото́ть absent.';
    return;
  end if;

  if v_good_id is null then
    update lemmas set form = 'хоте́ть', updated_at = now() where id = v_bad_id;
    raise notice 'CORRECTION : хото́ть renommé en хоте́ть (id=%)', v_bad_id;
    return;
  end if;
  ...
  if exists (
    ...
    raise exception 'ABANDON : conflit UNIQUE(user_id, lemma_id) хото́ть↔%', v_good_form;
  end if;
  ...
  if exists (
    ...
    raise exception 'ABANDON : texts.content_annotated référence encore хото́ть — remap JSONB manuel requis';
  end if;

  delete from lemmas where id = v_bad_id;
  raise notice 'FUSION : хото́ть → % (keep=%)', v_good_form, v_good_id;
end $$;

select id, form from lemmas where form in ('хото́ть', 'хоте́ть', 'хотеть') order by form;
