-- ============================================================
-- B3 cas 4 — свиде́ние → свида́ние (faute de lettre)
-- Surface rattachée : свида́ния! — cible linguistique confirmée.
-- En base live : свида́ние ABSENT ; « свидание » (nu) présent
-- → fusion vers la nue si l'accentuée manque.
-- ============================================================
...
begin
  select id into v_bad_id from lemmas where form = 'свиде́ние';
  select id, form into v_good_id, v_good_form from lemmas where form = 'свида́ние';
  if v_good_id is null then
    select id, form into v_good_id, v_good_form from lemmas where form = 'свидание';
  end if;

  if v_bad_id is null then
    raise notice 'RAPPORT : lemme fautif свиде́ние absent — rien à faire.';
    return;
  end if;
  ...
  raise notice 'RAPPORT-AVANT свиде́ние id=% → cible % id=% | wf=% lcl=% uv=% ec=% lk=%',
    v_bad_id, coalesce(v_good_form, 'свида́ние (rename)'), v_good_id, v_wf, v_lcl, v_uv, v_ec, v_lk;
end $$;
...
begin
  ...
  if v_bad_id is null then
    raise notice 'SKIP : свиде́ние absent.';
    return;
  end if;

  if v_good_id is null then
    update lemmas set form = 'свида́ние', updated_at = now() where id = v_bad_id;
    raise notice 'CORRECTION : свиде́ние renommé en свида́ние (id=%)', v_bad_id;
    return;
  end if;
  ...
  if exists (
    ...
    raise exception 'ABANDON : conflit UNIQUE(user_id, lemma_id) свиде́ние↔%', v_good_form;
  end if;
  ...
  if exists (
    ...
    raise exception 'ABANDON : texts.content_annotated référence encore свиде́ние — remap JSONB manuel requis';
  end if;

  delete from lemmas where id = v_bad_id;
  raise notice 'FUSION : свиде́ние → % (keep=%)', v_good_form, v_good_id;
end $$;

select id, form from lemmas where form in ('свиде́ние', 'свида́ние', 'свидание') order by form;
