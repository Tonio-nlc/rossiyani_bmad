-- ============================================================
-- HOMOLYPHE LATIN — знáть → знать (á latin précomposé U+00E1)
-- Surface cache : зна́ет,. Cible cyrillique « знать » déjà en base → fusion.
-- Ne pas créer « знáть » corrigé en U+0301 ici : la nue existe (canon. ultérieure séparée).
...
begin
  select id into v_bad_id from lemmas where form = 'знáть';

  select id, form into v_good_id, v_good_form from lemmas where form = 'знать';

  if v_bad_id is null then
    raise notice 'RAPPORT : lemme fautif знáть absent — rien à faire.';
    return;
  end if;
  ...
  raise notice 'RAPPORT-AVANT знáть id=% → cible % id=% | wf=% lcl=% uv=% ec=% lk=%',
    v_bad_id, coalesce(v_good_form, 'знать (rename)'), v_good_id,
...
begin
  ...
  if v_bad_id is null then
    raise notice 'SKIP : знáть absent.';
    return;
  end if;

  if v_good_id is null then
    update lemmas set form = 'знать', updated_at = now() where id = v_bad_id;
    raise notice 'CORRECTION : знáть renommé en знать (id=%)', v_bad_id;
    return;
  end if;
  ...
  if exists (
    ...
    raise exception 'ABANDON : conflit UNIQUE(user_id, lemma_id) знáть↔%', v_good_form;
  end if;
  ...
  if exists (
    ...
    raise exception 'ABANDON : texts.content_annotated référence encore знáть — remap JSONB manuel';
  end if;

  delete from lemmas where id = v_bad_id;
  raise notice 'FUSION : знáть → % (keep=%)', v_good_form, v_good_id;
end $$;

select id, form from lemmas where form in ('знáть', 'знать') order by form;
