-- ============================================================
-- HOMOLYPHE LATIN — двa → два (a latin U+0061)
-- Surface cache : два. Monosyllabe : forme nue « два ». Si absente → rename.
...
begin
  select id into v_bad_id from lemmas where form = 'двa';

  select id, form into v_good_id, v_good_form from lemmas where form = 'два';

  if v_bad_id is null then
    raise notice 'RAPPORT : lemme fautif двa absent — rien à faire.';
    return;
  end if;
  ...
  raise notice 'RAPPORT-AVANT двa id=% → cible % id=% | wf=% lcl=% uv=% ec=% lk=%',
    v_bad_id, coalesce(v_good_form, 'два (rename)'), v_good_id,
...
begin
  ...
  if v_bad_id is null then
    raise notice 'SKIP : двa absent.';
    return;
  end if;

  if v_good_id is null then
    update lemmas set form = 'два', updated_at = now() where id = v_bad_id;
    raise notice 'CORRECTION : двa renommé en два (id=%)', v_bad_id;
    return;
  end if;
  ...
  if exists (
    ...
    raise exception 'ABANDON : conflit UNIQUE(user_id, lemma_id) двa↔%', v_good_form;
  end if;
  ...
  if exists (
    ...
    raise exception 'ABANDON : texts.content_annotated référence encore двa — remap JSONB manuel';
  end if;

  delete from lemmas where id = v_bad_id;
  raise notice 'FUSION : двa → % (keep=%)', v_good_form, v_good_id;
end $$;

select id, form from lemmas where form in ('двa', 'два') order by form;
